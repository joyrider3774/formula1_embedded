...
"""Convert the skin images in assets to the flash data headers the game includes.
  ...
  assets/skins  -> source/formula1_embedded/images

Every image becomes a raw RGB565 little endian header (<name>_RGB565_LE.h, see png2rgb565.py).
None of them is run length encoded: the game redraws the parts of the background that
changed, so it has to be able to read any pixel of it straight away.
...
SOURCE_DIR = os.path.join(ROOT, "source", "formula1_embedded")

# every skin has to have these, the game includes all of them
IMAGES = ["background", "bigfont", "enemy", "lcdfont", "player"]


def header_name(png):
    """File name of the header a skin image converts to."""
    return png[:-4] + "_RGB565_LE.h"
